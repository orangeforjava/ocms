...
function tx_chk(obj){
	...
	if(key.indexOf("/11/")>-1&&(strLen(val)!=11)){
	...
	if(key.indexOf("/2-20")>-1&&(strLen(val)<2||strLen(val)>20)){
		oShow.innerHTML="长度必须2-20位"
		oShow.style.display=""
		return false
	}
	if(key.indexOf("/2-40")>-1&&(strLen(val)<2||strLen(val)>40)){
		oShow.innerHTML="长度必须2-40位"
		oShow.style.display=""
		return false
	}
