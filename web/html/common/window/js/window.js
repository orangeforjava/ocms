...
/*
  UI.State
    ...
    to bookmark an application in a given state.
    It uses fragment part of URL to store state information.
    A state is just a name for now, basic examples are "previewing", "editing" for a composition
    application.
    ...
    Inside the callback, event.memo.name gives the state name.
    'initial' is a special state which occurs when the user gets back to the initial page (with empty fragment).
...
UI.State = (function(browser) {
  ...
  function getFragment() {
    ...
    return fragment && fragment != '#' ? fragment.substr(1) : null;
  }

  function fragmentChanged(newFragment) {
    fragment = newFragment;
    if (fragment && fragment.include('state:'))
      document.fire('state:changed', { name: fragment.sub('state:', '') });
    else if (!fragment && changed)
      document.fire('state:changed', { name: 'initial' });
  }

  document.whenReady(function() {
    fragmentChanged(getFragment());
    ...
    if (browser.IE) {
      ...
      writeToIFrame(fragment || '');

      watch = function() {
        var newFragment = readFromIFrame();
      ...
      change = function(fragment) {
        ...
        fragments[history.length] = fragment;
      };
