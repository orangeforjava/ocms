 ...
 */
package org.openuap.cms.core.hander;
 ...
 * 
 * <p>
 * $Id: CmsMappingExceptionResolver.java 3764 2010-04-19 11:18:56Z
 * orangeforjava$
 * </p>
 * 
 * @author Joseph
 * @version 1.0
 */
public class CmsMappingExceptionResolver implements HandlerExceptionResolver,
		Ordered {
	...
	public void setDefaultErrorView(String defaultErrorView) {
		...
		if (logger.isInfoEnabled()) {
			logger
					.info("Default error view is '" + this.defaultErrorView
							+ "'");
		}
	}

	/**
	 * Specify the set of handlers that this exception resolver should map. The
	 * exception mappings and the default error view will only apply to the
	 * specified handlers.
	 * 
	 * <p>
	...
	public ModelAndView resolveException(HttpServletRequest request,
		...
		Exception shouldEx = ex;
		if (ex instanceof InvocationTargetException) {
			Throwable trowable = ((InvocationTargetException) ex)
					.getTargetException();
			if (trowable instanceof Exception) {
				shouldEx = (Exception) trowable;
			}
		}
		...
		if (this.exceptionMappings != null) {
			...
			for (Enumeration names = this.exceptionMappings.propertyNames(); names
					.hasMoreElements();) {
				String exceptionMapping = (String) names.nextElement();
				int depth = getDepth(exceptionMapping, shouldEx);
				if (depth >= 0 && depth < deepest) {
					deepest = depth;
					viewName = this.exceptionMappings
							.getProperty(exceptionMapping);
				}
			}
		}
